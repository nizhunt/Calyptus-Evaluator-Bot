...
export async function notifyEvaluationCompletion(payload) {
  const callbackUrl = payload?.is_test
    ? "https://dev.api.calyptus.co/api/ai-fluency-tests"
    : "https://api.calyptus.co/api/ai-fluency-tests";
  ...
  if (process.env.EVALUATION_COMPLETE_API_KEY) {
    headers.Authorization = `Bearer ${process.env.EVALUATION_COMPLETE_API_KEY}`;
  }
