...
export class ScreenRecorder {
    ...
    async handleRecordingStopped(apiBaseUrl) {
        ...
        try {
            ...
            const fallbackDurationSeconds = this.recorder.getElapsedTime();
            const recordingDurationSeconds = measuredDurationSeconds > 0
    ...
    async runTranscriptionInBackground({ apiBaseUrl, recordingId, blobUrl, audioChunkUrls, backupAudioUrl, }) {
        try {
            this.logTrace('transcription:bg_request_start', {
                recordingId,
                blobUrl,
                audioChunkUrlsCount: audioChunkUrls.length,
                hasBackupAudioUrl: Boolean(backupAudioUrl),
            });
            const transcriptionRes = await fetch(`${apiBaseUrl}/api/recordings/${recordingId}/transcription`, {
                method: 'POST',
            ...
            if (!transcriptionRes.ok) {
                this.logTrace('transcription:bg_request_failed', {
                    recordingId,
                    status: transcriptionRes.status,
                });
                return;
            }
            this.logTrace('transcription:bg_request_accepted', { recordingId });
            const pollStartedAt = Date.now();
            const maxPollMs = 15 * 60 * 1000;
            while (Date.now() - pollStartedAt < maxPollMs) {
                const statusRes = await fetch(`${apiBaseUrl}/api/recordings/${recordingId}`, {
                    headers: this.buildAuthHeaders(),
                    cache: 'no-store',
                });
                if (!statusRes.ok) {
                ...
                if (statusData.status === 'transcript_ready') {
                    const transcriptText = statusData.transcriptText || '';
                    this.logTrace('transcription:bg_ready', {
                        recordingId,
                ...
                if (statusData.status === 'error') {
                    this.logTrace('transcription:bg_status_error', {
                        recordingId,
            ...
            this.logTrace('transcription:bg_timeout', { recordingId });
        }
        catch (error) {
            this.logTraceError('transcription:bg_failed', error, { recordingId });
        }
    }
    ...
    mountOverlayUI() {
        ...
        // Ensure the burned-in PIP matches the on-screen bubble even before any drag.
