...
export class UploadManager {
    ...
    /**
     * Upload a single video chunk to blob storage.
     * Returns the URL on success, null on failure (does not throw).
     */
    async uploadVideoChunk(blob, recordingId, chunkIndex, mimeType) {
        const ext = mimeType?.includes('mp4') ? 'mp4' : 'webm';
        const filename = `recording_${recordingId}_video_chunk_${String(chunkIndex).padStart(3, '0')}.${ext}`;
        const maxRetries = 2;
        let lastError = null;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                const result = await upload(filename, blob, {
                    access: 'public',
                    handleUploadUrl: this.handleUploadUrl,
                    clientPayload: JSON.stringify({ recordingId, asset: 'video-chunk', chunkIndex }),
                    contentType: mimeType || 'video/webm',
                });
                return result.url;
            } catch (error) {
                lastError = error;
                console.warn(`[UploadManager] Video chunk ${chunkIndex} upload attempt ${attempt} failed:`, error.message);
                if (attempt < maxRetries) {
                    await new Promise((resolve) => setTimeout(resolve, Math.pow(2, attempt - 1) * 1000));
                }
            }
        ...
        console.warn(`[UploadManager] Video chunk ${chunkIndex} upload failed after ${maxRetries} retries:`, lastError?.message);
        return null;
    }
