...
export class UploadManager {
    ...
    async upload(blob, mimeType) {
        ...
        try {
            ...
            console.log('[UploadManager] Upload complete:', result.url);
            return {
                url: result.url,
    ...
    async uploadWithRetry(blob, mimeType, maxRetries = 3) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                console.log(`[UploadManager] Upload attempt ${attempt}/${maxRetries}`);
                ...
                if (attempt < maxRetries) {
                    ...
                    console.log(`[UploadManager] Retrying in ${delay}ms...`);
