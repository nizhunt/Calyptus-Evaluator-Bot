    ...
    async uploadVideoChunk(blob, recordingId, chunkIndex, mimeType) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                lastError = error;
                console.warn(`[UploadManager] Video chunk ${chunkIndex} attempt ${attempt} failed:`, error.message);
                if (attempt < maxRetries) {
                    await new Promise((resolve) => setTimeout(resolve, Math.pow(2, attempt - 1) * 1000));
                }
            }
        ...
        console.warn(`[UploadManager] Video chunk ${chunkIndex} failed after ${maxRetries} retries:`, lastError?.message);
        return null;
    }
