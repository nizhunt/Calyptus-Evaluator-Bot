...
export function buildStoredEvaluationRecord({ id, payload }) {
  ...
  const sourceTestId = textOrNil(
    firstDefined(
      metadata?.sourceTestId,
      metadata?.testRequestId,
      metadata?.externalTestId
    )
  );
  const isTest = Boolean(firstDefined(metadata?.is_test, metadata?.isTest, false));
  ...
  return {
    ...
    metadata: {
      ...
      recordingDurationSeconds:
        toNumberOrNull(firstDefined(metadata.recordingDurationSeconds)) ?? 0,
